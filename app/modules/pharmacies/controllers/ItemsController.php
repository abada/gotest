...
class ItemsController extends Controller
{
            ...
            if(Yii::$app->request->isAjax){
                ...
                if (isset($_FILES) && $this->module->settings['itemThumb']) {
                    ...
                    if ($model->image && $model->validate(['image'])) {
                        $model->image = Image::upload($model->image, 'pharmacies');
                    } else {
                        $model->image = '';
                    }
                }
                if ($model->save()) {
                    $this->flash('success', Yii::t('easyii/pharmacies', 'Item created'));
    ...
    public function actionEdit($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if ($model->save()) {
                    $this->flash('success', Yii::t('easyii/pharmacies', 'Item updated'));
                    return $this->redirect(['/admin/'.$this->module->id.'/items/edit', 'id' => $model->primaryKey]);
                } else {
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('easyii/pharmacies', 'Item deleted'));
    }
    ...
    private function generateForm($fields, $data = null)
    {
        ...
        foreach($fields as $field)
        {
            ...
            elseif ($field->type === 'select') {
                $options = ['' => Yii::t('easyii/pharmacies', 'Select')];
